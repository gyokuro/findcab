...
func TestMongoDbQuery(test *testing.T) {
	testQuery(mongodb, test, locations[0], 1000., []findcab.Cab{cabs[0]})
	testQuery(mongodb, test, locations[0], 500., []findcab.Cab{})
}

func TestMongoDbDelete(test *testing.T) {
	testUpsert(mongodb, test) // make sure data is there
	testQuery(mongodb, test, locationOf(cabs[1]), 1000., []findcab.Cab{cabs[1]})
	...
	testQuery(mongodb, test, locationOf(cabs[1]), 1000., []findcab.Cab{})
}

func TestMongoDbDeleteAll(test *testing.T) {
	testDeleteAll(mongodb, test)
	testQuery(mongodb, test, locationOf(cabs[0]), 1000., []findcab.Cab{})
	testQuery(mongodb, test, locationOf(cabs[1]), 1000., []findcab.Cab{})
