...
func TestSimpleQuery(test *testing.T) {
	testQuery(simple, test, locations[0], 1000., []findcab.Cab{cabs[0]})
	testQuery(simple, test, locations[0], 500., []findcab.Cab{})
}

func TestSimpleDelete(test *testing.T) {
	testUpsert(simple, test) // make sure data is there
	testQuery(simple, test, locationOf(cabs[1]), 1000., []findcab.Cab{cabs[1]})
	...
	testQuery(simple, test, locationOf(cabs[1]), 1000., []findcab.Cab{})
}

func TestSimpleDeleteAll(test *testing.T) {
	testDeleteAll(simple, test)
	testQuery(simple, test, locationOf(cabs[0]), 1000., []findcab.Cab{})
	testQuery(simple, test, locationOf(cabs[1]), 1000., []findcab.Cab{})
