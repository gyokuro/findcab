...
func TestHaversine(test *testing.T) {
	...
	expected := 0.341
	d := Haversine(p1, p2, findcab.Miles)
	if !check(expected, d, 3) {
		test.Error("Expect in miles", expected, d)
	}

	d = Haversine(p2, p1, findcab.Miles)
	if !check(expected, d, 3) {
		test.Error("Expect in miles", expected, d)
	}

	expected = d * 5280.
	d = Haversine(p1, p2, findcab.Feet)
	if !check(expected, d, 0) {
		test.Error("Expect in feet", expected, d)
	}

	expected = 0.549
	d = Haversine(p1, p2, findcab.Kilometers)
	if !check(expected, d, 3) {
		test.Error("Expect in km", expected, d)
	}

	expected = 549.
	d = Haversine(p1, p2, findcab.Meters)
	if !check(expected, d, 0) {
